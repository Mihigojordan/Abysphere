...
export class AdminController {
  constructor(private readonly adminServices: SuperAdminService) { }

  @Post('register')
  async registerByClient(@Body() req) {
    try {
      return await this.adminServices.registerSuperAdmin(req);
    } catch (error) {
  ...
  async login(@Body() body: { adminEmail: string; password: string }, @Res() res: Response) {
    try {
      const loginResult = await this.adminServices.superAdminLogin(body);
  ...
  */
  ...
  async getAdminProfile(@Req() req: RequestWithAdmin) {
    const adminId = req.admin?.id as string;
    try {
      const admin = await this.adminServices.findSuperAdminById(adminId);
  ...
  async HostLocking(@Req() req: RequestWithAdmin) {
    ...
    try {
      return await this.adminServices.lockSuperAdmin(adminId);
    } catch (error) {
  ...
  async adminUnlocking(@Req() req: RequestWithAdmin, @Body() datas) {
    ...
    try {
      return await this.adminServices.unlockSuperAdmin(adminId, datas);
    } catch (error) {
  ...
  async updateAdmin(
    ...
    return this.adminServices.updateSuperAdmin(id, body);
  }

  @Delete(':id')
  async deleteAdmin(@Param('id') id: string) {
    return this.adminServices.deleteSuperAdmin(id);
  }
}
