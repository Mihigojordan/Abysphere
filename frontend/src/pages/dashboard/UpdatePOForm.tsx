...
import purchaseOrderService, { type UpdatePODto, type CreatePOItemDto } from '../../services/purchaseOrderService';
...
const UpdatePOForm: React.FC = () => {
    ...
    const [formData, setFormData] = useState<UpdatePODto>({
        supplierId: '',
    ...
    // Load PO data + suppliers in parallel
    useEffect(() => {
        const load = async () => {
            setIsFetching(true);
            try {
                const [po, supplierList] = await Promise.all([
                    purchaseOrderService.getOne(id!, token),
    ...
    const handleSubmit = async () => {
        ...
        setIsLoading(true);
        try {
            await purchaseOrderService.update(id!, formData, token);
            showToast('Purchase Order updated successfully', 'success');
            setTimeout(() => navigate(`/${role}/dashboard/purchase-management`), 1500);
        } catch (error: any) {
            showToast(error.response?.data?.message || 'Failed to update PO', 'error');
        } finally {
            setIsLoading(false);
        }
    };
                ...
                <div className="px-6 py-4 flex items-center justify-between max-w-[1400px] mx-auto">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p className="text-[10px] font-bold text-primary-600 uppercase tracking-[0.2em] mt-0.5">
                                {poNumber}
                            </p>
                        </div>
                    </div>
