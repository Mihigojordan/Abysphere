...
interface Props {
    ...
    stock: StockIn | null;
    onClose: () => void;
    onUpdate: (id: string, addedQuantity: number, expiryDate?: string) => Promise<void>;
}

const QuickUpdateModal: React.FC<Props> = ({ isOpen, stock, onClose, onUpdate }) => {
    ...
    const newTotal = (stock.quantity || 0) + purchasedQuantity;
            ...
            <div className="bg-white rounded-xl shadow-xl max-w-sm w-full p-6">
                ...
                <div className="flex items-center gap-3 p-3 bg-primary-50 rounded-lg mb-4">
                    ...
                    <div className="min-w-0">
                        <p className="font-medium text-sm text-gray-900 truncate">{stock.productName}</p>
                        <p className="text-xs text-gray-600">Current stock: {stock.quantity} {stock.unit}</p>
                        {stock.unitPrice && (
                            <p className="text-xs text-gray-500">Unit price: Rwf {Number(stock.unitPrice).toLocaleString()}</p>
                        )}
                    </div>
                </div>
                ...
                <div className="mb-4">
                    <label className="block text-xs font-medium text-gray-700 mb-1.5">
                        ...
                        Purchased Quantity ({stock.unit || 'PCS'})
                    </label>
                    <input
                        type="number"
                        min="1"
                    ...
                    {purchasedQuantity > 0 && (
                        <p className="text-xs text-green-600 mt-1">
                            New total: {stock.quantity} + {purchasedQuantity} = <strong>{newTotal} {stock.unit || 'PCS'}</strong>
                        </p>
                    )}
                </div>
