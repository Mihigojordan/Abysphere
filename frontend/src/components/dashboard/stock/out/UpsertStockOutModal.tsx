...
const UpsertStockOutModal: React.FC<UpsertStockOutModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm overflow-y-auto p-4 sm:p-6">
      <div className="bg-theme-bg-primary rounded-2xl w-full max-w-6xl my-auto max-h-[92vh] flex flex-col shadow-2xl border border-theme-border animate-in fade-in zoom-in duration-300">
        <div className="flex justify-between items-center p-6 border-b border-theme-border">
          ...
          <button
            onClick={onClose}
            className="p-2 hover:bg-theme-bg-tertiary rounded-xl transition-all text-theme-text-secondary hover:text-theme-text-primary hover:rotate-90"
            type="button"
          >
            <X size={24} />
          </button>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          {isUpdateMode ? (
            <div className="space-y-6">
              <div className="flex items-center gap-3 p-4 bg-theme-bg-tertiary rounded-2xl border border-theme-border">
                <div className="relative flex items-center">
                  <input
                    ...
                    className="h-5 w-5 rounded-lg border-theme-border bg-theme-bg-primary text-primary-500 focus:ring-primary-500/20 transition-all cursor-pointer"
                  />
                </div>
                <label htmlFor="isExternal" className="text-[11px] font-black text-theme-text-primary uppercase tracking-widest cursor-pointer">
            ...
            <div className="bg-theme-bg-tertiary border border-theme-border rounded-2xl p-6">
              <div className="flex justify-between items-center mb-6">
                <h3 className="text-lg font-black text-theme-text-primary uppercase tracking-widest">Sale Items</h3>
              </div>
              ...
              {formData.salesEntries.map((entry, i) => {
                ...
                return (
                  <div key={i} className="bg-theme-bg-primary border border-theme-border rounded-2xl p-6 mb-6 shadow-xl animate-in slide-in-from-bottom-2 duration-300">
                    <div className="flex justify-between items-center mb-6">
                      <div className="flex items-center gap-6">
                        <span className="text-[11px] font-black text-theme-text-secondary uppercase tracking-widest">Item {i + 1}</span>
                        {/* Per-row Toggle */}
                        <div className="flex items-center gap-2 px-3 py-1.5 bg-theme-bg-tertiary rounded-xl border border-theme-border">
                          <input
                            type="checkbox"
                            id={`isExternal-${i}`}
              ...
              <button
                ...
                className="w-full py-4 border-2 border-dashed border-theme-border text-theme-text-secondary hover:text-primary-500 hover:border-primary-500/50 hover:bg-primary-500/5 rounded-2xl transition-all font-black text-[10px] uppercase tracking-widest mt-4"
              >
                + Add Another Item
              </button>
            </div>
          ...
          <div className="border-t border-theme-border pt-8 mt-4">
            <h3 className="text-lg font-black text-theme-text-primary uppercase tracking-widest mb-6">Client Information (Optional)</h3>
          ...
          {!isUpdateMode && formData.salesEntries.some(e => e.stockinId && e.quantity) && (
            <div className="border-t border-theme-border pt-8 mt-4">
              <h3 className="text-lg font-black text-theme-text-primary uppercase tracking-widest mb-6">Transaction Summary</h3>
              <div className="bg-theme-bg-tertiary rounded-2xl p-6 border border-theme-border space-y-4 shadow-xl">
                <div className="flex justify-between items-center">
                  <span className="text-[10px] font-black text-theme-text-secondary uppercase tracking-widest">Total Items</span>
                  <span className="text-sm font-black text-theme-text-primary">
                    {formData.salesEntries.filter(e => e.stockinId).length}
                  </span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-[10px] font-black text-theme-text-secondary uppercase tracking-widest">Total Quantity</span>
                  <span className="text-sm font-black text-theme-text-primary">
                ...
                {formData.salesEntries.some(e => e.soldPrice) && (
                  <div className="flex justify-between items-center pt-4 border-t border-theme-border">
                    <span className="text-[11px] font-black text-theme-text-secondary uppercase tracking-widest">Estimated Total</span>
                    <span className="text-2xl font-black text-primary-500 tracking-tighter">
                      {formData.salesEntries
                        .filter(e => e.soldPrice && e.quantity)
                        .reduce((sum, e) => sum + (Number(e.soldPrice) * Number(e.quantity)), 0)
                        .toLocaleString()} <span className="text-xs uppercase ml-1">RWF</span>
                    </span>
                  </div>
                )}
              </div>
          ...
          {/* Action Buttons */}
          <div className="flex flex-col sm:flex-row gap-4 pt-8 mt-4 border-t border-theme-border">
            <button
              type="button"
              onClick={onClose}
              className="flex-1 py-4 border-2 border-theme-border rounded-xl hover:bg-theme-bg-tertiary transition-all font-black text-[10px] uppercase tracking-widest text-theme-text-secondary hover:text-theme-text-primary"
            >
              Cancel
            </button>
            <button
              ...
              className="flex-[2] py-4 bg-primary-500 text-white rounded-xl hover:bg-primary-600 shadow-lg shadow-primary-500/20 disabled:opacity-50 disabled:cursor-not-allowed transition-all font-black text-[10px] uppercase tracking-widest flex items-center justify-center gap-2"
            >
              {isLoading ? (
                <>
                ...
                isUpdateMode ? "Update Sale Record" : "Complete Transaction"
              )}
            </button>
          </div>
        </form>
        ...
        <div className="mt-8 p-6 bg-primary-500/5 rounded-2xl border border-primary-500/10">
          <h4 className="text-[11px] font-black text-primary-500 mb-4 flex items-center gap-2 uppercase tracking-widest">
            <AlertCircle size={16} />
            Tips for Best Results
          </h4>
          <ul className="text-[10px] font-bold text-theme-text-secondary uppercase tracking-widest space-y-2.5 ml-1">
            <li className="flex items-start gap-2">
              <span className="w-1.5 h-1.5 rounded-full bg-primary-500 mt-1 flex-shrink-0" />
              Select items from the dropdown to add to the sale
            </li>
            <li className="flex items-start gap-2">
              <span className="w-1.5 h-1.5 rounded-full bg-primary-500 mt-1 flex-shrink-0" />
            ...
            <li className="flex items-start gap-2">
              <span className="w-1.5 h-1.5 rounded-full bg-primary-500 mt-1 flex-shrink-0" />
              Selling price is required for external items
            </li>
            <li className="flex items-start gap-2">
              <span className="w-1.5 h-1.5 rounded-full bg-primary-500 mt-1 flex-shrink-0" />
              System will warn if selling below cost price
            </li>
            <li className="flex items-start gap-2">
              <span className="w-1.5 h-1.5 rounded-full bg-primary-500 mt-1 flex-shrink-0" />
              All quantity values must be whole numbers
            </li>
            <li className="flex items-start gap-2">
              <span className="w-1.5 h-1.5 rounded-full bg-primary-500 mt-1 flex-shrink-0" />
              Client information is optional for your records
            </li>
          </ul>
        </div>
      </div>
