...
import { X, Package, MapPin, Calendar, DollarSign, AlertTriangle, Tag, Layers, Clock, Store } from 'lucide-react';
...
const ViewStockInModal: React.FC<Props> = ({ isOpen, stock, onClose }) => {
    ...
    const totalValue = stock.quantity * (stock.unitPrice || 0);
            ...
            <div className="bg-white rounded-xl shadow-xl max-w-lg w-full max-h-[90vh] overflow-y-auto">
                ...
                <div className="flex items-center justify-between p-5 border-b border-gray-100">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h3 className="text-lg font-semibold text-gray-900">{stock.productName}</h3>
                            {stock.sku && <p className="text-xs text-gray-500">SKU: {stock.sku}</p>}
                        </div>
                    </div>
                ...
                <div className="p-5 space-y-4">
                    <div className="grid grid-cols-2 gap-4">
                        <DetailItem
                            ...
                            label="Quantity"
                            value={`${stock.quantity} ${stock.unit || 'PCS'}`}
                            highlight={isLowStock}
                        />
                        <DetailItem
                            icon={<AlertTriangle className="w-4 h-4 text-orange-500" />}
                            label="Reorder Level"
                            value={stock.reorderLevel ? `${stock.reorderLevel} ${stock.unit || 'PCS'}` : 'Not set'}
                        />
                        <DetailItem
                            icon={<DollarSign className="w-4 h-4 text-green-500" />}
                            label="Unit Price"
                            value={formatCurrency(stock.unitPrice)}
                        />
                        <DetailItem
                        ...
                        <DetailItem
                            ...
                            label="Category"
                            value={stock.stockcategory?.name || 'N/A'}
                        />
                        <DetailItem
                    ...
                    {stock.description && (
                        <div className="pt-3 border-t border-gray-100">
                            ...
                            <div className="text-sm text-gray-700" dangerouslySetInnerHTML={{ __html: stock.description }} />
                        </div>
                    )}
