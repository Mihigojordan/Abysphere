// useSupplierOfflineSync.ts
import { useEffect, useCallback, useState, useRef } from 'react';
import { supplierSyncService } from '../services/sync/supplierSyncService';
...
interface UseSupplierOfflineSyncOptions {
  autoSync?: boolean;
  syncInterval?: number;
  enableDebugLogs?: boolean;
}

interface UseSupplierOfflineSyncReturn {
...
// === Hook ===
export const useSupplierOfflineSync = (
  options: UseSupplierOfflineSyncOptions = {}
): UseSupplierOfflineSyncReturn => {
  ...
  const log = (message: string, data?: any): void => {
    if (enableDebugLogs) {
      console.log(`[useSupplierOfflineSync] ${message}`, data ?? '');
    }
  };

  const updateSyncStatus = useCallback(async (): Promise<void> => {
    try {
      const stats = await supplierSyncService.getSyncStatus();
      setSyncStatus((prev) => ({
        ...prev,
        stats,
        syncError: null,
      ...
      log('Failed to get supplier sync status:', error);
      setSyncStatus((prev) => ({
        ...prev,
        stats: {
          totalSuppliers: 0,
  ...
  const triggerSync = useCallback(
    async (force = false): Promise<SyncResult> => {
      if (!isOnline) {
        log('Offline - skipping supplier sync');
      ...
      try {
        const result = force
          ? await supplierSyncService.forceSync()
        ...
        log('Supplier sync failed', error);
  ...
  useEffect(() => {
    log('useSupplierOfflineSync hook initialized');

    // Setup auto-sync listeners
    if (autoSync) {
      supplierSyncService.setupAutoSync();
    }
  ...
  return {
    ...
    totalSuppliers: syncStatus.stats?.totalSuppliers ?? 0,
    unsyncedSuppliers: syncStatus.stats?.unsyncedSuppliers ?? 0,
